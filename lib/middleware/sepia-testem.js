...
var SepiaProcessFactory = require('../subprocess-factories/sepia');
...
var SepiaTestemMiddleware = CoreObject.extend({
  ...
  createSepiaProcess: function() {
    var sepiaFactory = new SepiaProcessFactory;
    var sepiaCommands = {
      sepiaPort: process.env.SEPIA_PORT.toString(),
      fixtures:  process.env.SEPIA_FIXTURES,
      vcrMode:   process.env.VCR_MODE
    };
    if(process.env.SEPIA_CONFIG_FILE) {
      sepiaCommands['sepiaConfigFile'] = process.env.SEPIA_CONFIG_FILE
    }
    var sepia = sepiaFactory.spawn(sepiaCommands);
